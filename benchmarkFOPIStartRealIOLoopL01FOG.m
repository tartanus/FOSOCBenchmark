...
warning('off','all')
repetitions=2;
%change the path for your current folder
pathName="C:\Users\jairo\Dropbox\doctorado UC\yangquan assignments\DT peltier\DT SOC\benchmarkFOPIDataIOL01FO\IODataFO";
% alpha=1.5;
for k=2:repetitions
...
%     kiRandAux=alphaStableNoise(alpha,1000)*maxKi;
    ...
    lambdaRandAux=ffGn(1000,alpha,0.05,0.8);
    kpRandAux=abs(kpRandAux/(max(abs(kpRandAux)))*maxKp);
    kiRandAux=abs(kiRandAux/(max(abs(kiRandAux)))*maxKi);
    lambdaRandAux=abs(lambdaRandAux/(max(abs(lambdaRandAux))));
    
    for z=1:length(lambdaRandAux)
        
        if lambdaRandAux(z)<lambdaO
            lambdaRandAux(z)=lambdaO;
        end
    end
...
%     randomNoiseOutput=[timeVecFinal' randNoise(1:length(timeVecFinal))];
    ...
    simData=sim("FOPIDSOCOptimRand.slx",'SrcWorkspace','current');
