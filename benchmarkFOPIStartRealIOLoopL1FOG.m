...
%constraints limits
OVLim=5;                %overshoot
TSLim=50;               %Settling time limit (s)
...
warning('off','all')
repetitions=2;
pathName="C:\Users\jairo\Dropbox\doctorado UC\yangquan assignments\DT peltier\DT SOC\benchmarkFOPIDataIOL1FO\IODataFO";
% alpha=1.5;
for k=2:repetitions
...
%     randomNoiseOutput=[timeVecFinal' randNoise(1:length(timeVecFinal))];
    ...
    simData=sim("FOPIDFOSOCOptimRand1.slx",'SrcWorkspace','current');
    benchmarkFiguresFOSOCRepeat
    execTimeIter=toc
    %saving data
    tic
